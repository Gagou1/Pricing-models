...
#dS_t=r*S_t*dt+sigma*S_t*dW_t
    ...
    def createPath(self,steps,T):
        BScDiffusion=np.zeros(steps+1, dtype=float)
        BScDiffusion[0]=self.S0
        Vol=self.sigma*np.ones(steps+1,dtype=float)
        
        for j in range(1,steps+1):
            BScDiffusion[j]=(BScDiffusion[j-1]+BScDiffusion[j-1]*self.r*(T/steps)
                              +BScDiffusion[j-1]*self.sigma*(math.sqrt(T)*np.random.normal()/math.sqrt(steps)))
...
#dv_t=kappa*(vb-v_t)*dt+epsilon*sqrt(v_t)*(rho*dW^1_t+sqrt(1-rho^2)dW^2_t)
    ...
    def __correlatedBrownian(self,steps):
        
        B1=np.random.normal(0,1,steps)
    ...
    def createPath(self,steps,T):
        HestDiffusion=np.zeros(steps+1, dtype=float)
        HestDiffusion[0]=self.S0
        Vol=np.zeros(steps+1, dtype=float)
        Vol[0]=self.v0
        B1=self.__correlatedBrownian(steps)[0]
        B2=self.__correlatedBrownian(steps)[1]
        #Generate the volatility
        
        for j in range(1,steps+1):
            Vol[j]=(Vol[j-1]+self.kappa*(self.vb-Vol[j-1])*(T/steps)
                    +self.epsilon*math.sqrt(max(Vol[j-1],0))*math.sqrt(T)/math.sqrt(steps)*B2[j-1])
        #Generate the stock price
        
        for j in range(1,steps+1):
            HestDiffusion[j]=(HestDiffusion[j-1]+self.r*HestDiffusion[j-1]*(T/steps)
                             +math.sqrt(max(Vol[j-1],0))*HestDiffusion[j-1]*math.sqrt(T)/math.sqrt(steps)*B1[j-1]) 
        return [HestDiffusion,Vol]
